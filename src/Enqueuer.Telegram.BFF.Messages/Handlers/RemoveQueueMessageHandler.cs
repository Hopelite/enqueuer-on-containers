    ...
    ILocalizationProvider localizationProvider) : IMessageHandler
{
    ...
    public Task HandleAsync(MessageContext messageContext, CancellationToken cancellationToken)
    {
        ...
        {
            var errorMessage = _localizationProvider.GetMessage(MessageKeys.RemoveQueueErrorMissingQueueName, new MessageParameters(messageContext.Chat.Culture));
        ...
        return _queueingClient.DeleteQueueAsync(messageContext.Chat.Id, queueContext.QueueName, cancellationToken);
    }
}
